...
export default function Books({ books, searchText, baseUrl, className }) {
    ...
    const handleSearch = (ev) => {
        ...
        // gest search keyword
        /** @type {String} */
        const searchKeyword = new FormData(ev.target)
            .get("search-keyword")
            .toLowerCase();
    ...
    return (
        <div className="">
            <form
                className="mt-10 mb-5 flex flex-col sm:flex-row gap-2"
                onSubmit={handleSearch}
            >
                <TextField
                    onChange={(ev) => {
                        useSearchInputText(ev.target.value);
                    }}
            ...
            <div
                className={`grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 sm:gap-10 ${className}`}
            >
